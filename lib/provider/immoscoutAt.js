...
async function getListings(url) {
  // Pass null as waitForSelector so puppeteerExtractor returns full page.content()
  // instead of the innerHTML of a single element. The JSON-LD blocks we need are
  // in <head>, not inside the listing container, so we must have the full HTML.
...
function normalize(o) {
  ...
  // Prices may have an "ab " prefix (e.g. "ab 279.000 €") — strip non-numeric
  // leading characters before passing to extractNumber.
  const priceStr = (o.price ?? '').replace(/^[^\d]*/, '');
  return {
    id,
    link: o.link,
    title: (o.title || '').replace('NEU', '').trim(),
    price: extractNumber(priceStr),
...
const config = {
  ...
  getListings,
  // Custom getListings still needs a shared/proxied Puppeteer browser, unlike
  // providers driving the request via a plain fetch/API call.
  usesPuppeteer: true,
