...
async function fetchDetails(listing, browser) {
  try {
    const html = await puppeteerExtractor(listing.link, null, { browser, name: 'wgGesuchtAt_details' });
    ...
    logger.warn(`Could not fetch wgGesuchtAt detail page for listing '${listing.id}'.`, error?.message || error);
    return listing;
  }
}
...
function applyBlacklist(o) {
  return o.id != null && !isOneOf(o.title, appliedBlackList) && !isOneOf(o.description, appliedBlackList);
}
...
const config = {
  ...
  puppeteerOptions: {
    preNavigateUrl: 'https://www.wg-gesucht.at/',
  },
  crawlFields: {
    id: '@data-id',
...
export const metaInformation = {
  name: 'WG-Gesucht AT',
  baseUrl: 'https://www.wg-gesucht.at/',
  id: 'wgGesuchtAt',
};
