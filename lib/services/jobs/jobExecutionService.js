...
export function initJobExecutionService({ providers, settings, intervalMs }) {
  ...
  async function executeJob(job) {
    ...
    try {
      ...
      // disables the proxy. Routing the headless browser through a residential
      // proxy avoids datacenter-IP based bot detection on the Puppeteer-based
      // providers (immowelt, immonet, kleinanzeigen, immoscoutAt, ...).
      ...
      for (const prov of jobProviders) {
        try {
          ...
          if (!browser && (matchedProvider.config.getListings == null || matchedProvider.config.usesPuppeteer)) {
