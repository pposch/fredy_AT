...
describe('#immoscoutAt testsuite()', () => {
  provider.init(providerConfig.immoscoutAt, []);

  let browser;

  beforeAll(async () => {
    browser = await launchBrowser(providerConfig.immoscoutAt.url);
  }, TEST_TIMEOUT);
    ...
    async () => {
      ...
      return await new Promise((resolve, reject) => {
        ...
        fredy.execute().then((listings) => {
          ...
          expect(notificationObj.serviceName).toBe('immoscoutAt');
          notificationObj.payload.forEach((notify) => {
            ...
            expect(notify.link).toContain('https://www.immobilienscout24.at/');
          });
          resolve();
        });
      });
