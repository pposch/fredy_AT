...
import * as provider from '../../lib/provider/immoweltAt.js';
...
describe('#immoweltAt testsuite()', () => {
  let browser;
  let liveListings;

  beforeAll(async () => {
    browser = await launchBrowser(providerConfig.immoweltAt.url);
  }, TEST_TIMEOUT);
  ...
  it(
    'should test immoweltAt provider',
    async () => {
      ...
      provider.init(providerConfig.immoweltAt, [], []);
      ...
      expect(notificationObj.serviceName).toBe('immoweltAt');
      notificationObj.payload.forEach((notify) => {
        ...
        expect(notify.link).toContain('https://www.immowelt.at');
  ...
  describe('with provider_details enabled', () => {
    it(
      ...
      async () => {
        ...
        expect(enriched.link).toContain('https://www.immowelt.at');
